...
export default function Contact() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(msg.includes("already being processed")
        ? msg
  ...
  return (
    <section id="contact" className="py-24 relative overflow-hidden" style={{ background: "linear-gradient(150deg, #022c22 0%, #064e3b 45%, #059669 100%)" }}>
        ...
        <ScrollReveal>
          <div className="text-center mb-12">
            ...
            <h2 className="text-4xl sm:text-5xl font-black text-white leading-tight mb-4">
              Apply to open your <span className="bg-amber-400 text-emerald-900 px-2 rounded-lg">school account</span>
            </h2>
            ...
            <div className="grid md:grid-cols-5">
              ...
              <div className="md:col-span-2 p-8 flex flex-col relative overflow-hidden bg-white/8 border-r border-white/10">
                ...
                <div className="relative flex flex-col h-full">
                  ...
                  <p className="text-emerald-100/65 text-sm font-semibold leading-relaxed mb-7">
                  ...
                  <div className="space-y-2.5 mb-auto">
                    {bullets.map((b) => (
                      <div key={b.text} className="flex items-center gap-3 bg-white/8 rounded-2xl px-4 py-3 border border-white/10">
                        <FlatIcon name={b.icon} size={15} className="text-emerald-400 shrink-0" />
                ...
                {submitted ? (
                  <div className="flex flex-col items-center justify-center h-full py-16 text-center">
                    <div className="w-20 h-20 bg-emerald-100 rounded-3xl flex items-center justify-center mb-6">
                      <FlatIcon name="check-circle" size={40} className="text-emerald-500" />
                    </div>
                    <h3 className="text-2xl font-black text-stone-900 mb-2">Application received!</h3>
                    <p className="text-stone-500 font-semibold max-w-xs text-sm leading-relaxed">
                      Our team will review your application and contact you within 24 hours to complete your school setup.
                    </p>
                    <div className="mt-8 flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-2xl px-5 py-3">
                      <FlatIcon name="check-circle" size={16} className="text-emerald-500" />
                      <span className="text-emerald-700 text-sm font-bold">We&apos;ll be in touch soon</span>
                    </div>
                  </div>
                ) : (
                  <>
                    <h3 className="text-2xl font-black text-stone-900 mb-1">School account application</h3>
                    <p className="text-stone-400 text-sm font-semibold mb-7">Complete the form below. We review every application and reach out within 24 hours.</p>
                    ...
                    <form onSubmit={handleSubmit} className="grid sm:grid-cols-2 gap-4">
                      {[
                        ...
                        <div key={f.label}>
                          <label className="block text-xs font-black text-stone-500 mb-1.5 uppercase tracking-wide">{f.label}</label>
                          <input
                            name={f.name}
                            type={f.type}
                            required
                            ...
                            className="w-full border-2 border-stone-200 bg-stone-50 rounded-xl px-4 py-3 text-sm font-semibold text-stone-800 focus:outline-none focus:border-emerald-400 focus:ring-4 focus:ring-emerald-100 transition-all placeholder:text-stone-300 disabled:opacity-60"
                          />
                        </div>
                      ))}
                      <div>
                        <label className="block text-xs font-black text-stone-500 mb-1.5 uppercase tracking-wide">Your role</label>
                        <select name="role" required disabled={loading} className="w-full border-2 border-stone-200 bg-stone-50 rounded-xl px-4 py-3 text-sm font-semibold text-stone-800 focus:outline-none focus:border-emerald-400 focus:ring-4 focus:ring-emerald-100 transition-all disabled:opacity-60">
                      ...
                      <div className="sm:col-span-2">
                        <label className="block text-xs font-black text-stone-500 mb-1.5 uppercase tracking-wide">Message (optional)</label>
                        <textarea name="message" rows={3} disabled={loading} placeholder="e.g. number of students, modules you need, questions..." className="w-full border-2 border-stone-200 bg-stone-50 rounded-xl px-4 py-3 text-sm font-semibold text-stone-800 focus:outline-none focus:border-emerald-400 focus:ring-4 focus:ring-emerald-100 resize-none transition-all placeholder:text-stone-300 disabled:opacity-60" />
                      </div>
                      <div className="sm:col-span-2">
                        <button
                          type="submit"
                          disabled={loading}
                          className="btn-fun w-full bg-amber-400 hover:bg-amber-300 text-amber-900 font-black py-4 rounded-full text-base flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                        >
                          {loading ? (
                            <>
                        ...
                        <p className="text-center text-stone-400 text-xs font-semibold mt-3 flex items-center justify-center gap-1.5">
